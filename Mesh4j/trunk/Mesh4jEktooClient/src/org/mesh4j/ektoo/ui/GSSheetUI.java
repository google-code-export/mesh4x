...
public class GSSheetUI extends JPanel
{
	...
	private GSSheetModel model = null;
  ...
  public GSSheetUI(GSSheetUIController controller)
  {
	...
	public JComboBox getTableList() 
	{
		if (listTable == null) {
			...
			listTable.addItemListener(new ItemListener() 
			{
				...
				{
					getController().changeWorksheetName( (String)listTable.getSelectedItem());
					...
					if (sheetIndex != -1)
					{
						SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() 
						{
							...
							{
								setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
								setList(getUser() , getPass(), getKey(), (String)listTable.getSelectedItem());
								return null;
					        }
					        public void done() 
					        {
	...
	public JComboBox getColumnList() {
			...
			listColumn.addItemListener(new java.awt.event.ItemListener() 
      {
        ...
        {
          getController().changeUniqueColumnPosition(listColumn.getSelectedIndex());
          getController().changeUniqueColumnName((String)listColumn.getSelectedItem());
        }
      });
	...
	public void setList(String user, String pass, String key, String sheetName)
	{
		...
		IGoogleSpreadSheet spreadsheet = new GoogleSpreadsheet(key, user, pass);
		GSWorksheet<GSRow<GSCell>> workSheet = spreadsheet.getGSSpreadsheet().getGSWorksheetBySheetName(sheetName);
