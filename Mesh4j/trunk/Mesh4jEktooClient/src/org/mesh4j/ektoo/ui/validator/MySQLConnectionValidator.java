...
{ 
  ...
  private boolean validate(MySQLAdapterModel mysqlModel)
  {
  ...
  private boolean validate(JComponent form)
  {
    ...
    {
      setError( ui.getUserText(), EktooUITranslator.getErrorEmptyOrNull(MySQLUIController.USER_NAME_PROPERTY));
      isValid = false;
    }
    
    if (new String(ui.getPassText().getPassword()).trim().length() == 0)
    {
      setError( ui.getPassText(), EktooUITranslator.getErrorEmptyOrNull(MySQLUIController.USER_PASSWORD_PROPERTY));
      isValid = false;
    }

    if (ui.getHostText().getText().trim().length() == 0)
    {
      setError( ui.getHostText(), EktooUITranslator.getErrorEmptyOrNull(MySQLUIController.HOST_NAME_PROPERTY));
      isValid = false;
    }

    if (ui.getPortText().getText().trim().length() == 0)
    {
      setError( ui.getPortText(), EktooUITranslator.getErrorEmptyOrNull(MySQLUIController.PORT_NO_PROPERTY));
      isValid = false;
    }   
    
    if (ui.getDatabaseText().getText().trim().length() == 0)
    {
      setError( ui.getDatabaseText(), EktooUITranslator.getErrorEmptyOrNull(MySQLUIController.DATABASE_NAME_PROPERTY));
      isValid = false;
    }   
