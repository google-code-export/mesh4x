...
public class ConflictsUI extends JPanel implements IErrorListener{
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
			        ...
			        // ui
			        ConflictsUI ui = new ConflictsUI(null, null, rdfSchema, conflicts);
					ui.setVisible(true);
	...
	public ConflictsUI(EktooFrame ui, ISyncAdapter syncAdapter, IRDFSchema rdfSchema, List<Item> conflictingItems) {
		super();
		this.ownerUI = ui;
		...
		buttonSave.addActionListener(new ActionListener(){
			@Override public void actionPerformed(ActionEvent e) {
				ResolveConflictsTask task = new ResolveConflictsTask(ownerUI, adapter, conflicts, getWinners());
				task.execute();
			}			
		});
		...
		//panelTable.setFont(new Font("", Font.PLAIN, 16));
	...
	private void makeConflictPanel(IRDFSchema rdfSchema, Item item) {
		...
		//panelEdit.setFont(new Font("", Font.PLAIN, 16));
