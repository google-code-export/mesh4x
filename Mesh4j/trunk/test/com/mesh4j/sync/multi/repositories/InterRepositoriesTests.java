package com.mesh4j.sync.multi.repositories;
...
import com.mesh4j.sync.feed.FeedRepository;
import com.mesh4j.sync.feed.FeedWriter;
import com.mesh4j.sync.feed.ItemXMLContent;
import com.mesh4j.sync.feed.atom.AtomSyndicationFormat;
import com.mesh4j.sync.feed.rss.RssSyndicationFormat;
import com.mesh4j.sync.hibernate.HibernateRepository;
...
public class InterRepositoriesTests {
	...
	public void shouldSyncFeed2Hibernate() throws DocumentException{
		...
		Feed feed = new Feed()
			.addItem(new Item(new ItemXMLContent(id1, id1, id1, e1), new Sync(id1).update(Security.getAuthenticatedUser(), new Date())))
			.addItem(new Item(new ItemXMLContent(id2, id2, id2, e2), new Sync(id2).update(Security.getAuthenticatedUser(), new Date())));
		
		FeedRepository feedRepo = new FeedRepository(feed);
