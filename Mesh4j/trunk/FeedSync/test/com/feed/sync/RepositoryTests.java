...
public class RepositoryTests {
	...
	public void ShouldAddAndGetItem() {
		...
		Sync sync = getBehaviors().create(xml.getId(), "kzu", TestHelper.now(),
				false);
		// TODO (?): sync.ItemHash = xml.hashCode();
		...
		Assert.assertEquals(item.getModelItem().getTitle(), saved
				.getModelItem().getTitle());
		Assert.assertEquals(item.getModelItem().getDescription(), saved
				.getModelItem().getDescription());
		Assert.assertEquals(item.getModelItem().getId(), saved.getModelItem()
				.getId());
	...
	public void ShouldGetAllIfNullWhen() {
		...
		String id = TestHelper.newID();
		IModelItem modelItem =new XmlItem(id, "foo", "bar", TestHelper.makeElement("<payload />"));
		Sync sync = getBehaviors().create(id, by, null, false);
		Item item = new Item(modelItem, sync);
		repo.add(item);

		id = TestHelper.newID();
		modelItem = new XmlItem(id, "foo", "bar", TestHelper.makeElement("<payload />"));
	...
	public void ShouldGetAllPassFilter() {
		...
		Predicate<Item> filter = new IdPredicate(id);

		String id2 = TestHelper.newID();
		item = new Item(new XmlItem(id2, "foo", "bar", TestHelper
	...
	public void ShouldGetAllSinceRemoveMilliseconds() {
		...
		XmlItem item = new XmlItem(TestHelper.newID(), "foo", "bar", TestHelper
				.makeElement("<payload />"));
		Sync sync = getBehaviors().create(item.getId(), "kzu", created, false);
		// TODO (?): sync.ItemHash = item.hashCode();
	...
	public void ShouldGetAllWithFilterPassToImplementation() {
		...
		Predicate<Item> filter = new NullPredicate<Item>();
		repo.getAll(filter);
	...
	public void ShouldResolveConflicts() {
		...
		Item item = new Item(new NullModelItem(id), getBehaviors().update(sync,
				"kzu", TestHelper.nowSubtractHours(2), false));
		sync.getConflicts().add(
				new Item(new XmlItem(TestHelper.newID(), "foo", "bar",
	...
	public void ShouldSaveUpdatedItemOnResolveConflicts() {
		...
		XmlItem xml = (XmlItem) item.getModelItem().clone();
		...
		((XmlItem) item.getModelItem()).setTitle("Resolved");

		repo.update(item, true);

		Item storedItem = repo.get(item.getModelItem().getId());

		Assert.assertEquals("Resolved", storedItem.getModelItem().getTitle());
	}
	...
	public void ShouldSaveUpdatedItemOnResolveConflicts2() {
		...
		Assert
				.assertEquals(
						...
						"Resolved", storedItem.getModelItem().getTitle());
	}
	...
	public void ShouldResolveConflictsPreserveDeletedState() {
		...
		repo.update(new Item(item.getModelItem(), deletedSync), true);

		Item saved = repo.get(item.getModelItem().getId());
		...
		Assert
				.assertTrue(
						...
						saved.getModelItem() == null
								|| saved.getModelItem() instanceof NullModelItem);
		Assert
				.assertTrue(
	...
	private class IdPredicate implements Predicate<Item>{
		...
		public boolean evaluate(Item obj) {
