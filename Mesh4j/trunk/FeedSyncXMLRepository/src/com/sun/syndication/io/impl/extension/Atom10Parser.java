...
public class Atom10Parser extends BaseWireFeedParser {
    ...
    protected WireFeed parseFeed(Element eFeed) throws FeedException {
        ...
        feed.setAlternateLinks(parseAlternateLinks(null, baseURI, eList));
        feed.setOtherLinks(parseOtherLinks(null, baseURI, eList));
        ...
        if (eList.size()>0) {
            feed.setEntries(parseEntries(baseURI, eList));
        }
    ...
    private Link parseLink(Entry entry, String baseURI, Element eLink) {
        Link link = new Link();
    ...
    private List parseAlternateLinks(Entry entry, String baseURI, List eLinks) {
        List links = new ArrayList();
        for (int i=0;i<eLinks.size();i++) {
            Element eLink = (Element) eLinks.get(i);
            Link link = parseLink(entry, baseURI, eLink);
            if (link.getRel() == null
    ...
    private List parseOtherLinks(Entry entry, String baseURI, List eLinks) {
        List links = new ArrayList();
        for (int i=0;i<eLinks.size();i++) {
            Element eLink = (Element) eLinks.get(i);
            Link link = parseLink(entry, baseURI, eLink);
    ...
    protected List parseEntries(String baseURI, List eEntries) {
        List entries = new ArrayList();
        for (int i=0;i<eEntries.size();i++) {
            entries.add(parseEntry((Element)eEntries.get(i), baseURI));
        }
        return (entries.size()>0) ? entries : null;
    }
    
    public Entry parseEntry(Element root, Element eEntry) throws FeedException {
        String baseURI = null;
        try {
            baseURI = findBaseURI(root);
        } catch (Exception e) {
            throw new FeedException("ERROR while finding base URI of feed", e);
        }
    	return parseEntry(eEntry, baseURI);
    }

    protected Entry parseEntry(Element eEntry, String baseURI) {
        ...
        entry.setAlternateLinks(parseAlternateLinks(entry, baseURI, eList));
        entry.setOtherLinks(parseOtherLinks(entry, baseURI, eList));
