...
public class RDFInstance {
	...
	protected static RDFInstance buildFromRDFXml(RDFSchema schema, String rdfXml) {
		...
		if(schema instanceof SchemaMappedRDFSchema){
			return buildFromRDFXml((SchemaMappedRDFSchema)schema, XMLHelper.parseElement(rdfXml));
		}
	...
	//my test: revert
	//revert the element(syncSchema format) to original source format (using syncSchema and schemaConversionMap)
	protected static RDFInstance buildFromRDFXml(SchemaMappedRDFSchema schema, Element element) {
		Guard.argumentNotNull(element, "element");

		Map<String, Resource> syncSchema = schema.getSyncSchema();
		Map<String, String> schemaRevertMap = schema.getSchemaRevertMap();
		Map<String, String> schemaConvertMap = schema.getSchemaConvertMap();
		
		Guard.argumentNotNull(syncSchema, "schema->syncSchema");
		Guard.argumentNotNull(schemaRevertMap, "schema->schemaRevertMap");
		Guard.argumentNotNull(schemaConvertMap, "schema->schemaConvertMap");
			
		if(!(schemaConvertMap.get(schema.getName()) != null 
				&& syncSchema.get(schemaConvertMap.get(schema.getName())).getURI().equals(XSD.ENTITY.getURI()))){
			throw new MeshException("Invalid or unmapped entity");
		}
		
		Map<String, Object> propertyValues = new HashMap<String, Object>();
		for(Object el : element.element(schemaConvertMap.get(schema.getName())).elements()){
			String propName = ((Element)el).getName();
			Object propValue = ((Element)el).getStringValue();
			propertyValues.put(schemaRevertMap.get(propName), propValue);
		}
		
		StringBuffer sb = new StringBuffer("");
		for(String idProp : schema.getIdentifiablePropertyNames()){
			if (propertyValues.get(idProp) !=null)
				sb.append(propertyValues.get(idProp)+",");
		}
		
		return buildFromProperties((RDFSchema)schema, sb.toString().substring(0, sb.toString().length()-1), propertyValues);
	}
	...
	protected static RDFInstance buildFromPlainXML(RDFSchema rdfSchema, String id, Element element, Map<String, ISchemaTypeFormat> typeFormats, String[] splitElements){
		...
		if(rdfSchema instanceof SchemaMappedRDFSchema){
			return buildFromPlainXML((SchemaMappedRDFSchema)rdfSchema, id, element, typeFormats, splitElements);
		}
	...
	//my test: convert
	//convert the element(source) to syncSchema format (using syncSchema and schemaConversionMap)
	protected static RDFInstance buildFromPlainXML(SchemaMappedRDFSchema rdfSchema, String id, Element element, Map<String, ISchemaTypeFormat> typeFormats, String[] splitElements){
		Guard.argumentNotNull(rdfSchema, "rdfSchema");
		Guard.argumentNotNullOrEmptyString(id, "id");
		Guard.argumentNotNull(element, "element");
		Guard.argumentNotNull(typeFormats, "typeFormats");
		
		Map<String, Resource> syncSchema = rdfSchema.getSyncSchema();
		Map<String, String> schemaConvertMap = rdfSchema.getSchemaConvertMap();
		
		//create a copy of RDFSchema from rdfSchema with domain class as element.getName()
		if(!(schemaConvertMap.get(element.getName()) != null 
				&& syncSchema.get(schemaConvertMap.get(element.getName())).getURI().equals(XSD.ENTITY.getURI()))){
			throw new MeshException("Invalid or unmapped entity");
		}
		
		RDFSchema convertedSchema = new RDFSchema(schemaConvertMap.get(element.getName()), "http://mesh4x/"+schemaConvertMap.get(element.getName())+"#", schemaConvertMap.get(element.getName()));
		
		//convert id properties
		List<String> mappedPropNames = new ArrayList<String>();
		List<String> idProps = rdfSchema.getIdentifiablePropertyNames();
		for(String idProp:idProps){
			if(schemaConvertMap.get(idProp) != null)
			mappedPropNames.add(schemaConvertMap.get(idProp));
		}
		if(mappedPropNames.size()>0)
			convertedSchema.setIdentifiablePropertyNames(mappedPropNames);
		
		//convert guid properties
		mappedPropNames.clear();
		
		List<String> guidProps = rdfSchema.getGUIDPropertyNames();
		for(String idProp:guidProps){
			if(schemaConvertMap.get(idProp) != null)
			mappedPropNames.add(schemaConvertMap.get(idProp));
		}
		if(mappedPropNames.size()>0)
			convertedSchema.setGUIDPropertyNames(mappedPropNames);
		
		//convert version properties
		if(schemaConvertMap.get(rdfSchema.getVersionPropertyName())!=null)
			convertedSchema.setVersionPropertyName(schemaConvertMap.get(rdfSchema.getVersionPropertyName()));
		
		//convert all general properties
		for(DatatypeProperty dp : rdfSchema.getDomainProperties()){
			//convertedSchema.addStringProperty( dp.getLocalName(), dp.getLabel(IRDFSchema.DEFAULT_LANGUAGE), IRDFSchema.DEFAULT_LANGUAGE);
			if(schemaConvertMap.get(dp.getLocalName())!=null){
				Resource resource = syncSchema.get(schemaConvertMap.get(dp.getLocalName()));
				//keep the original label that might be useful later on
				convertedSchema.addProperty(schemaConvertMap.get(dp.getLocalName()), dp.getLabel(IRDFSchema.DEFAULT_LANGUAGE), IRDFSchema.DEFAULT_LANGUAGE, /*dp.getRDFType()*/resource);
			}
		}
		
		RDFInstance instance = new RDFInstance(convertedSchema, "uri:urn:"+id);
		...
		List<DatatypeProperty> domainProperties = rdfSchema.getDomainProperties();
		//List<DatatypeProperty> domainProperties = convertedSchema.getDomainProperties();
		for (DatatypeProperty dataTypeProperty : domainProperties) {
			
			dataTypeName = dataTypeProperty.getLocalName();
			
			String mappedPropertyName = schemaConvertMap.get(dataTypeName);
			if(mappedPropertyName == null) continue; //ignore the fields that are not mapped
			
			fieldElement = getFieldElement(dataTypeName, element, splitElements);
			if(fieldElement != null){
				fieldValue = fieldElement.getText();
				
				//convert type of fieldValueToMap as appropriate

				Resource resource = syncSchema.get(mappedPropertyName);
				RDFDatatype dataType = TypeMapper.getInstance().getTypeByName(resource.getURI());
				//RDFDatatype dataType = TypeMapper.getInstance().getTypeByName(dataTypeProperty.getRange().getURI());
				if(dataType.isValid(fieldValue)){
					try{
		            	if (dataType.getURI().equals(XSD.dateTime)) {
							ISchemaTypeFormat format = typeFormats.get(dataType.getURI());
							if(format != null)
								instance.setProperty(mappedPropertyName, format.parseObject(fieldValue));
							else 
								instance.setProperty(mappedPropertyName, dataType.parse(fieldValue));
						} else
							instance.setProperty(mappedPropertyName,
									convertedSchema.cannonicaliseValue(mappedPropertyName, fieldValue));
					...
					System.out.println("Incompatible field vale: "+fieldValue+" for mapped field: "+mappedPropertyName+" for type: "+ dataType.getURI());
					return null;
				}
			}
		}
...
//		if(rdfSchema instanceof SchemaMappedRDFSchema){
//			return buildFromProperties((SchemaMappedRDFSchema)rdfSchema, id, propertyValues);
//		}
	 ...
	 * @return
	 */
	protected static RDFInstance buildFromProperties(SchemaMappedRDFSchema rdfSchema, String id, Map<String, Object> propertyValues){
		Guard.argumentNotNull(rdfSchema, "rdfSchema");
		Guard.argumentNotNullOrEmptyString(id, "id");
		Guard.argumentNotNull(propertyValues, "propertyValues");
		...
		Map<String, Resource> syncSchema = rdfSchema.getSyncSchema();
		Map<String, String> schemaConvertMap = rdfSchema.getSchemaConvertMap();
		Map<String, String> schemaRevertMap = rdfSchema.getSchemaRevertMap();
		
		//create a copy of RDFSchema from rdfSchema with domain class as element.getName()
		if(!(schemaConvertMap.get(rdfSchema.getName()) != null 
				&& syncSchema.get(schemaConvertMap.get(rdfSchema.getName())).getURI().equals(XSD.ENTITY.getURI()))){
			throw new MeshException("Invalid or unmapped entity");
		}
		
		RDFSchema convertedSchema = new RDFSchema(schemaConvertMap.get(rdfSchema.getName()), "http://mesh4x/"+schemaConvertMap.get(rdfSchema.getName())+"#", schemaConvertMap.get(rdfSchema.getName()));
		...
		for (DatatypeProperty dataTypeProperty : domainProperties) {
			...
			dataTypeName = dataTypeProperty.getLocalName();
			fieldValue = propertyValues.get(schemaRevertMap.get(dataTypeName));
			if(fieldValue != null){
				instance.setProperty(dataTypeName, convertedSchema.cannonicaliseValue(dataTypeName, fieldValue));
			}
		}
		return instance;
	}
