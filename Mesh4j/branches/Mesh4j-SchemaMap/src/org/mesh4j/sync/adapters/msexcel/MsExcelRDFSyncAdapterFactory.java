...
public class MsExcelRDFSyncAdapterFactory{
	...
	/**
	 * This is to work with different schema
	 * @param sheetName
	 * @param idColumnNames
	 ...
	 * @param rdfBaseURL
	 * @param syncSchema
	 * @param schemaConversionMap
	 * @return
	 */
	protected static MsExcelContentAdapter createContentAdapter(String sheetName, String[] idColumnNames, String lastUpdateColumnName, IMsExcel excel, String rdfBaseURL, Map<String, Resource> syncSchema, Map<String, String> schemaConversionMap) {
		Guard.argumentNotNull(idColumnNames, "idColumnNames");
		if(idColumnNames.length == 0){
			Guard.throwsArgumentException("idColumnNames");
		}
		...
		Guard.argumentNotNullOrEmptyCollection(schemaConversionMap, "schemaConversionMap");
		...
		IRDFSchema rdfSchema = MsExcelToRDFMapping.extractRDFSchema(excel, sheetName, idColumnNames, lastUpdateColumnName, rdfBaseURL, syncSchema, schemaConversionMap);
	...
	public static SplitAdapter createSyncAdapter(IMsExcel excel, String sheetName, String[] idColumnNames, String lastUpdateColumnName, IIdentityProvider identityProvider, String rdfBaseURL,
		...
		String entityName = MsExcelToRDFMapping.getEntityName(sheetName);
		MsExcelSyncRepository syncRepo =  new MsExcelSyncRepository(excel, entityName+"_sync", identityProvider, IdGenerator.INSTANCE);
		MsExcelContentAdapter contentAdapter = createContentAdapter(sheetName, idColumnNames, lastUpdateColumnName, excel, rdfBaseURL, syncSchema, schemaConversionMap);
		SplitAdapter splitAdapter = new SplitAdapter(syncRepo, contentAdapter, identityProvider);
		return splitAdapter;
	}
