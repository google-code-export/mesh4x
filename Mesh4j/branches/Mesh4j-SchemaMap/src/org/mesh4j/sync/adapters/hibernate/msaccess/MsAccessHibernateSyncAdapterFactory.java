...
public class MsAccessHibernateSyncAdapterFactory implements ISyncAdapterFactory {
	...
	//my test
	public SplitAdapter createSyncAdapterFromFile(String sourceAlias, String mdbFileName, String tableName, IIdentityProvider identityProvider,	Map<String, Resource> syncSchema, Map<String, String> schemaConversionMap) {
		return createHibernateAdapter(mdbFileName, tableName, this.rdfBaseUri, this.baseDirectory, identityProvider, syncSchema, schemaConversionMap);
	}
	...
	private static SplitAdapter createSplitAdapter(String dbURL, String tableName, String user, String password, String contentMappingFileName, String syncMappingFileName, IRDFSchema rdfSchema, IIdentityProvider identityProvider) {
		...
		HibernateContentAdapter contentAdapter = new HibernateContentAdapter(builder, entityName);
	...
	public SplitAdapter createSyncAdapter(String sourceAlias, String sourceDefinition, IIdentityProvider identityProvider) throws Exception {
		return createSyncAdapter(sourceAlias, sourceDefinition, identityProvider, null, null);
	}
	
	//my test
	public SplitAdapter createSyncAdapter(String sourceAlias, String sourceDefinition, IIdentityProvider identityProvider,
			Map<String, Resource> syncSchema, Map<String, String> schemaConversionMap) throws Exception {
		String mdbFileName = getFileName(sourceDefinition);
		String tableName = getTableName(sourceDefinition);
		SplitAdapter msAccessAdapter = createSyncAdapterFromFile(sourceAlias, mdbFileName, tableName, identityProvider, syncSchema, schemaConversionMap);
	...
	public void changeSourceDefinition(String sourceAlias, String sourceDefinition, ISyncAdapter syncAdapter) {
		try{
			...
			if(this.rdfBaseUri != null){
				rdfSchema = MsAccessRDFSchemaGenerator.extractRDFSchema(mdbFileName, tableName, this.rdfBaseUri);
			}
	...
	public static SplitAdapter createHibernateAdapter(String mdbFileName, String tableName, String rdfBaseUri, String baseDirectory, IIdentityProvider identityProvider) {
		return createHibernateAdapter(mdbFileName, tableName, rdfBaseUri, baseDirectory, identityProvider, null, null);
	}
	...
	public static SplitAdapter createHibernateAdapter(String mdbFileName, String tableName, String rdfBaseUri, String baseDirectory, IIdentityProvider identityProvider,
		...
		try{
			...
			IRDFSchema rdfSchema = null;
			if(rdfBaseUri != null){
				rdfSchema = MsAccessRDFSchemaGenerator.extractRDFSchema(mdbFileName, tableName, rdfBaseUri, syncSchema, schemaConversionMap);
			}
