...
public class MsExcelToRDFMapping implements IMsExcelToXMLMapping{
	...
	public void appliesRDFToRow(Workbook wb, Sheet sheet, Row row, RDFInstance rdfInstance) {
		...
		for (int i = 0; i < size; i++) {
			...
			String propertyLabel = rdfInstance.getPropertyLabel(i);
			propertyValue = rdfInstance.getPropertyValue(propertyName);
				
			//my test
			if(propertyValue == null){
				propertyValue = rdfInstance.getPropertyValue(propertyLabel);
			}
			//my test	
			...
			if(propertyValue != null){
				...
				Cell headerCell = MsExcelUtils.getOrCreateCellStringIfAbsent(wb, headerRow, propertyLabel);
				//my test
