...
public class RDFInstance {
	...
	protected static RDFInstance buildFromPlainXML(RDFSchema rdfSchema, String id, String plainXML, Map<String, ISchemaTypeFormat> typeFormats){
		...
		while(it.hasNext()){
			...
			if(fieldElement == null){
				String lang = "en";
				String dataTypeName1 = dataTypeProperty.getLabel(lang);
				fieldElement = element.element(dataTypeName1);
			}
			//my test			
	...
	protected static RDFInstance buildFromProperties(RDFSchema rdfSchema, String id, Map<String, Object> propertyValues){
		...
		while(it.hasNext()){
			...
			if(fieldValue == null){
				String lang = "en";
				String dataTypeName1 = dataTypeProperty.getLabel(lang);
				fieldValue = propertyValues.get(dataTypeName1);
			}
			//my test
	...
	public String getPropertyLabel(int index) {
		DatatypeProperty domainObjectProperty = (DatatypeProperty)this.model.listDatatypeProperties().toList().get(index);
		return domainObjectProperty.getLabel("en");
	}
