...
public class CleanKMLTask extends AbstractKmlTask {

	public CleanKMLTask(KmlFrame kmlFrame, String fileName){
		super(kmlFrame, fileName);
	}

	protected void basicProcess(){
		String fileName = getFileName();
		setStatusInProcess(KmlUITranslator.getMessageCleanKML(fileName));
		...
		try{
			...
			domAdapter.clean();
			setStatusOk(KmlUITranslator.getMessageCleanKMLCompleted(KmlUITranslator.getMessageCleanKMLSuccessfuly()));	
		} catch (MeshException e) {
			LogFrame.Logger.error(e.getMessage(), e);
			setStatusError(KmlUITranslator.getMessageCleanKMLCompleted(KmlUITranslator.getMessageCleanKMLFailed()));
		}
	}
