...
public class MeshUI {
	...
	public MeshUI() throws Exception {
		...
		this.consoleNotification = new ConsoleNotification(
				this.textAreaConsole, this.imageStatus, this);
		this.consoleNotification.setReadyImageStatus();
		this.startUpSyncEngine();
	}

	private void initializeModem() {
		if (propertiesProvider.getDefaultPort().length() > 0
				&& propertiesProvider.getDefaultBaudRate() > 0) {
			modem = ModemHelper.getModem(propertiesProvider.getDefaultPort(),
					propertiesProvider.getDefaultBaudRate());
		}
	}
	...
	private void initializeProperties() throws Exception {
		...
		this.sourceIdResolver = new SourceIdMapper(this.propertiesProvider
				.getBaseDirectory()
	...
	protected void startUpSyncEngine() throws Exception {
		...
		if (modem != null
				&& !modem.getManufacturer().equals(
						MeshUITranslator.getLabelDemo())) {

			this.syncEngine = SyncEngineUtil
					.createSyncEngine(
							this.sourceIdResolver,
							modem,
							propertiesProvider.getBaseDirectory(),
							propertiesProvider.getDefaultSendRetryDelay(),
							propertiesProvider.getDefaultReceiveRetryDelay(),
							propertiesProvider.getDefaultMaxMessageLenght(),
							propertiesProvider.getIdentityProvider(),
							itemEncoding,
							propertiesProvider.getDefaultMessageEncoding(),
		...
		if (this.syncEngine == null) {
			this.syncEngine = SyncEngineUtil
					.createSyncEngineEmulator(
							...
							new ISmsConnectionInboundOutboundNotification[] { consoleNotification },
							new IMessageSyncAware[] { consoleNotification },
							false, inDir, outDir, myEndpointId);
		}
	}
	...
	private void createUI() {
		...
		frame.setIconImage(SwingResourceManager.getImage(MeshUI.class,
				"/cdc.gif"));
		...
		panelCommunications.setBorder(new TitledBorder(new BevelBorder(
				BevelBorder.RAISED), MeshUITranslator.getGroupCommunications(),
				TitledBorder.DEFAULT_JUSTIFICATION,
				TitledBorder.DEFAULT_POSITION, null, null));
		panelCommunications.setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("44dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("221dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("36dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, RowSpec.decode("12dlu"),
				FormFactory.DEFAULT_ROWSPEC, FormFactory.RELATED_GAP_ROWSPEC,
				FormFactory.DEFAULT_ROWSPEC, RowSpec.decode("9dlu"),
				RowSpec.decode("11dlu"), RowSpec.decode("14dlu"),
				FormFactory.RELATED_GAP_ROWSPEC }));
		...
		textFieldPhoneNumber.setToolTipText(MeshUITranslator
				.getToolTipPhoneNumber());
		panelCommunications.add(textFieldPhoneNumber, new CellConstraints(4, 5,
				3, 1, CellConstraints.FILL, CellConstraints.DEFAULT));
		textFieldPhoneNumber
				.setText(propertiesProvider.getDefaultPhoneNumber());

		labelSMSDevice = DefaultComponentFactory.getInstance().createLabel(
				MeshUITranslator.getLabelSMSDevice());
		panelCommunications.add(labelSMSDevice, new CellConstraints(2, 3,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));

		labelPhoneNumber = DefaultComponentFactory.getInstance().createLabel(
				MeshUITranslator.getLabelPhoneNumber());
		panelCommunications.add(labelPhoneNumber, new CellConstraints(2, 5,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));
		...
		comboSMSDevice.setModel(modem == null ? new DefaultComboBoxModel(
				new Modem[] { demoModem }) : new DefaultComboBoxModel(
				new Modem[] { modem }));
		comboSMSDevice.setToolTipText(modem == null ? demoModem.toString()
				: modem.toString());
		comboSMSDevice.addActionListener(deviceActionListener);
		panelCommunications.add(comboSMSDevice, new CellConstraints(4, 3,
		...
		buttonModemDiscovery.setToolTipText(MeshUITranslator
				.getToolTipAutoDetect());
		buttonModemDiscovery.addActionListener(modemDiscoveryActionListener);
		panelCommunications.add(buttonModemDiscovery, new CellConstraints(6, 3,
				CellConstraints.FILL, CellConstraints.FILL));

		labelUrl = new JLabel();
		labelUrl.setText(MeshUITranslator.getLabelURL());
		panelCommunications.add(labelUrl, new CellConstraints(2, 8,
				CellConstraints.RIGHT, CellConstraints.DEFAULT));

		textFieldURL = new JTextField();
		textFieldURL.setText(propertiesProvider.getDefaultURL());
		...
		panelCommunications.setFocusTraversalPolicy(new FocusTraversalOnArray(
				new Component[] { labelSMSDevice, labelPhoneNumber,
						textFieldPhoneNumber, buttonSynchronize }));

		final JPanel panelDataSource = new JPanel();
		panelDataSource.setBorder(new TitledBorder(new BevelBorder(
				BevelBorder.RAISED), MeshUITranslator.getLabelDataSource(),
				TitledBorder.DEFAULT_JUSTIFICATION,
				TitledBorder.DEFAULT_POSITION, null, null));
		panelDataSource
		...
		textFieldDataSource.setToolTipText(MeshUITranslator
				.getToolTipDataSource());
		textFieldDataSource.setText(propertiesProvider.getDefaultDataSource());
		panelDataSource.add(textFieldDataSource, new CellConstraints(2, 2, 3,
				1, CellConstraints.FILL, CellConstraints.FILL));

		ActionListener fileChooserFileActionListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String selectedFileName = openFileDialog(textFieldDataSource
						.getText());
		...
		buttonOpenFileDataSource.setToolTipText(MeshUITranslator
				.getToolTipFileChooser());
		buttonOpenFileDataSource
				.setText(MeshUITranslator.getLabelFileChooser());
		buttonOpenFileDataSource
				.addActionListener(fileChooserFileActionListener);
		panelDataSource.add(buttonOpenFileDataSource, new CellConstraints(6, 2,
				CellConstraints.FILL, CellConstraints.FILL));
		...
		panelFooter.setLayout(new FormLayout(new ColumnSpec[] {
				ColumnSpec.decode("37dlu"), FormFactory.RELATED_GAP_COLSPEC,
				ColumnSpec.decode("177dlu"), ColumnSpec.decode("5dlu"),
				ColumnSpec.decode("95dlu"), FormFactory.RELATED_GAP_COLSPEC },
				new RowSpec[] { RowSpec.decode("104dlu") }));
		frame.getContentPane().add(
				panelFooter,
				new CellConstraints(2, 8, CellConstraints.FILL,
						CellConstraints.FILL));
		...
		panelFooter.add(scrollPaneConsole, new CellConstraints(
				"2, 1, 5, 1, fill, fill"));
		...
		labelLogo.setIcon(SwingResourceManager.getIcon(MeshUI.class,
				"/Epi2002.jpg"));
		labelLogo.setText("");
		panelFooter.add(labelLogo, new CellConstraints(3, 1, 4, 1,
				CellConstraints.FILL, CellConstraints.FILL));

		imageStatus = new JLabel();
		imageStatus.setText("");
		panelFooter.add(imageStatus, new CellConstraints(1, 1,
		...
		panelExchange.setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("53dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("91dlu"),
		...
		panelExchange.add(buttonSynchronize, new CellConstraints(2, 2,
				CellConstraints.FILL, CellConstraints.FILL));
		...
		panelExchange.add(buttonCancel, new CellConstraints(4, 2,
				CellConstraints.FILL, CellConstraints.FILL));
		...
		ActionListener hiShowActionListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (scrollPaneConsole.isVisible()) {
					buttonHideShowConsole.setText(MeshUITranslator
							.getLabelHideConsole());
					buttonHideShowConsole.setToolTipText(MeshUITranslator
							.getToolTipHideConsole());
					buttonHideShowConsole1.setText(MeshUITranslator
							.getLabelHideConsole());
					buttonHideShowConsole1.setToolTipText(MeshUITranslator
							.getToolTipHideConsole());
					buttonHideShowConsole2.setText(MeshUITranslator
							.getLabelHideConsole());
					buttonHideShowConsole2.setToolTipText(MeshUITranslator
							.getToolTipHideConsole());
		...
		panelExchange.add(buttonHideShowConsole, new CellConstraints(6, 2,
				CellConstraints.CENTER, CellConstraints.FILL));

		JPanel panelMap = new JPanel(false);
		panelMap.setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("53dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("47dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("65dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				FormFactory.RELATED_GAP_ROWSPEC }));
		...
		panelSettings.setLayout(new FormLayout(new ColumnSpec[] {
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("57dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("78dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("72dlu"),
				FormFactory.RELATED_GAP_COLSPEC, ColumnSpec.decode("76dlu"),
				FormFactory.RELATED_GAP_COLSPEC }, new RowSpec[] {
				FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
				FormFactory.RELATED_GAP_ROWSPEC }));
		...
		panelSettings.add(buttonSaveDefaults, new CellConstraints(2, 2,
		...
		buttonDownloadSchema.setText(MeshUITranslator
				.getLabelDownloadMappings());
		buttonDownloadSchema.addActionListener(downloadSchemaActionListener);
		panelSettings.add(buttonDownloadSchema, new CellConstraints(4, 2,
				CellConstraints.FILL, CellConstraints.CENTER));
		...
		panelSettings.add(buttonClean, new CellConstraints(6, 2,
				CellConstraints.FILL, CellConstraints.CENTER));
		...
		tabbedPane.addTab(MeshUITranslator.getLabelTabDataExchange(),
				panelExchange);
		tabbedPane.addTab(MeshUITranslator.getLabelTabMap(), panelMap);
		tabbedPane
				.addTab(MeshUITranslator.getLabelTabSettings(), panelSettings);

		frame.getContentPane().add(tabbedPane, new CellConstraints(2, 6));
	}

	private Modem getDemoModem() {
		return new Modem("", 0, MeshUITranslator.getLabelDemo(), "", "", "", 0,
				0);
	}

	private ComboBoxModel getDataSourceTableModel() {
		try {
			Set<String> tableNames = MsAccessHelper
					.getTableNames(propertiesProvider.getDefaultDataSource());
			ComboBoxModel model = new DefaultComboBoxModel(tableNames.toArray());
			model.setSelectedItem(propertiesProvider.getDefaultTable());
			return model;
		} catch (Exception e) {
			Logger.error(e.getMessage(), e);
	...
	private class Task extends SwingWorker<Void, Void> {
		...
		public Void doInBackground() {
			...
			if (action == SYNCHRONIZE_SMS) {
				...
				SyncEngineUtil.addDataSource(sourceIdResolver, dataSource,
						tableName);
				...
				try {
					...
					String sourceAlias = sourceIdResolver.getSourceName(
							dataSource, tableName);
					SyncEngineUtil.synchronize(syncEngine,
							SyncMode.SendAndReceiveChanges,
							textFieldPhoneNumber.getText(), sourceAlias,
							propertiesProvider.getIdentityProvider(),
							propertiesProvider.getBaseDirectory(),
							sourceIdResolver);
					...
					syncInProcess = false;
					consoleNotification.setErrorImageStatus();
					consoleNotification.logError(t, MeshUITranslator
							.getLabelFailed());
				}
			}
			...
			if (action == SYNCHRONIZE_HTTP) {
				...
				try {
					...
					List<Item> conflicts = SyncEngineUtil.synchronize(
							textFieldURL.getText(), sourceIdResolver
									.getSourceName(dataSource, tableName),
							propertiesProvider, sourceIdResolver,
							SyncMode.SendAndReceiveChanges);
					consoleNotification.endSync(textFieldURL.getText(),
							sourceAlias, conflicts);
			...
			if (action == CANCEL_SYNC) {
				...
				SyncEngineUtil.cancelSynchronize(syncEngine,
						textFieldPhoneNumber.getText(), sourceIdResolver
								.getSourceName(dataSource, tableName));
				syncInProcess = false;
				consoleNotification.setEndSyncImageStatus();
			}
			...
			if (action == DISCOVERY_MODEMS) {
				consoleNotification.log(MeshUITranslator
						.getMessageBeginModemDiscovery());
				setModeDiscoveryModems();
				buttonModemDiscovery.setText(MeshUITranslator
						.getLabelStopModemDiscovery());
				buttonModemDiscovery.setToolTipText(MeshUITranslator
						.getToolTipStopAutoDetect());

				Modem[] modems = SyncEngineUtil
						.getAvailableModems(consoleNotification);
				consoleNotification.log(MeshUITranslator
						.getMessageEndModemDiscovery(modems.length));
			...
			if (action == CHANGE_DEVICE) {
				...
				try {
					...
					shutdownSyncEngine();
					consoleNotification.setErrorImageStatus();
					consoleNotification.logError(exc, MeshUITranslator
							.getLabelDeviceConnectionFailed(modem.toString()));
			...
			if (action == SAVE_DEFAULTS) {
				try {
					SyncEngineUtil.saveDefaults(modem, textFieldPhoneNumber
							.getText(), textFieldDataSource.getText(),
							(String) comboTables.getSelectedItem(),
...
//									.getIdentityProvider());
			...
			if (action == GENERATE_KML_WEB) {
				...
				try {
					...
					String fileName = propertiesProvider.getBaseDirectory()
							+ "/" + tableName + "_web.kml";

					SyncEngineUtil.makeKMLWithNetworkLink(propertiesProvider
							.getDefaultKMLTemplateNetworkLinkFileName(),
							fileName, tableName, urlWebKml);
					consoleNotification.setEndSyncImageStatus();
				} catch (Throwable t) {
					consoleNotification.setErrorImageStatus();
					consoleNotification.logError(t, MeshUITranslator
			...
			if (action == DOWNLOAD_MAPPINGS) {
				...
				try {
					SyncEngineUtil.downloadMappings(url, tableName,
							propertiesProvider);
					consoleNotification.setEndSyncImageStatus();
				} catch (Throwable t) {
					consoleNotification.setErrorImageStatus();
					consoleNotification.logError(t, MeshUITranslator
							.getLabelDownloadMappingsFailed());
				}
			}
			return null;
		}
	...
	private String openFileDialog(String fileName) {
		String fileNameSelected = openFileDialog(fileName,
				new FileNameExtensionFilter(MeshUITranslator
						.getLabelDataSourceFileExtensions(), "mdb"));
