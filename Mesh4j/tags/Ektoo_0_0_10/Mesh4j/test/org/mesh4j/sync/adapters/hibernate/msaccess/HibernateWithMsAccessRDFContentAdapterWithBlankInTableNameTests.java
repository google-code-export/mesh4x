...
public class HibernateWithMsAccessRDFContentAdapterWithBlankInTableNameTests {
	...
	private SplitAdapter makeAdapter(String fileName) {
		try{
			...
			SplitAdapter adapter = MsAccessHibernateSyncAdapterFactory.createHibernateAdapter(fileName, "table with blank name", "http://localhost:8080/mesh4x/feeds", TestHelper.baseDirectoryForTest(), NullIdentityProvider.INSTANCE);
			return adapter;
		}catch (Exception e) {
