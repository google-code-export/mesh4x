...
public class GetForMergeMessageProcessor implements IMessageProcessor {
	...
	// CONSTANTS 
	public final static String MESSAGE_TYPE = "4";
	...
	public Vector<IMessage> process(ISyncSession syncSession, IMessage message) {
		...
		if(syncSession.isOpen() && syncSession.getVersion() == message.getSessionVersion() && this.getMessageType().equals(message.getMessageType())){						
			...
			response.addElement(this.mergeMessage.createMessage(syncSession, syncId, diffHashCodes));
			return response;
		} else {
	...
	private int[] decodeDiffHashCodes(String data) {
		...
		if(st.hasMoreTokens()){
			...
			String[] diffHashCodes = DiffUtils.split(diffHashCodesString, IProtocolConstants.FIELD_SEPARATOR);
