...
public class MergeBehavior {
	...
	private static MergeResult mergeItems(Item localItem, Item incomingItem)
	{
		...
		//3.3.2    L
		mergeProcessParameters.setOuterCollection(new Vector<Item>());
		for (Item item : localItem.getSync().getConflicts()) {
			mergeProcessParameters.getOuterCollection().addElement(item);	
		}
		
		localItem.getSync().getConflicts().removeAllElements();
		mergeProcessParameters.getOuterCollection().addElement(localItem);

		//3.3.3   I
		mergeProcessParameters.setInnerCollection(new Vector<Item>());
		for (Item item : incomingItem.getSync().getConflicts()) {
			mergeProcessParameters.getInnerCollection().addElement(item);	
		}

		incomingItem.getSync().getConflicts().removeAllElements();
		mergeProcessParameters.getInnerCollection().addElement(incomingItem);

		//3.3.4  M
		mergeProcessParameters.setMergedCollection(new Vector<Item>());
		...
		if (!mergeProcessParameters.getWinner().getSync().isNoConflicts())
		{
			...
			for(Item z : mergeProcessParameters.getMergedCollection())
			{
				...
				{
					mergeProcessParameters.getWinner().getSync().getConflicts().addElement(z);
				}
			}
		}
	...
	{
		Vector<Item> resOuter = new Vector<Item>();
		for (Item item : mergeProcessParameters.getOuterCollection()) {
			resOuter.addElement(item);
		}
		
		Vector<Item> resInner = new Vector<Item>();
		for (Item item : mergeProcessParameters.getInnerCollection()) {
			resInner.addElement(item);
		}
		...
		for (Item x : mergeProcessParameters.getOuterCollection())
		{
			...
			if (!isSubsumed)
			{
				mergeProcessParameters.getMergedCollection().addElement(x);
				if (W == null)
				{
					W = x;
				}
	...
	private static Item firstWinsWithWhen(Item first, Item second)
	{
		...
		boolean firstWins = second.getLastUpdate().getWhen() == null ||
				(first.getLastUpdate().getWhen().getTime() > second.getLastUpdate().getWhen().getTime());
	...
	public static Item resolveConflicts(Item resolvedItem, String by, Date when, boolean deleteItem)
	{
		...
		Vector<Item> conflictItems = new Vector<Item>();
		for (Item item : Sr.getConflicts()) {
			conflictItems.addElement(item);	
		}
		...
		for (Item C : conflictItems)
		{
			...
			Vector<History> allConflictUpdatesHistories = new Vector<History>();
			int size = Sc.getUpdatesHistory().size();
			for (int i = 0; i < size; i++) {
				History history = Sc.getUpdatesHistory().elementAt(size - i -1);
				allConflictUpdatesHistories.addElement(history);
			}
		
			for (History Hc : allConflictUpdatesHistories)
			{
				boolean isSubsumed = false;
				int sizeSr = Sr.getUpdatesHistory().size();
				for (int i = 0; i < sizeSr; i++) {
					History Hr = Sr.getUpdatesHistory().elementAt(i);
