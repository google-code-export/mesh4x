...
public class Sync {
	...
	private Vector<Item> conflicts = new Vector<Item>();
	...
	public Sync removeConflict(Item item) {
		this.conflicts.removeElement(item);
		return this;
	}
	...
	public Sync clone(){
		...
		for (int i = 0; i < this.updatesHistory.size(); i++) {
			History history = this.updatesHistory.elementAt(i);
			newSync.updatesHistory.push(history.clone());
		}

		for (Item conflict : this.conflicts)
		{
			newSync.conflicts.addElement(conflict.clone());
		}
	...
	public boolean isSubsumedBy(Sync sync) {
		...
		for (int i = 0; i < sync.updatesHistory.size(); i++) {
			History Hy = sync.updatesHistory.elementAt(i);
	...
	public Sync addConflict(Item item) {
		this.getConflicts().addElement(item);
		return this;
	}
	...
	public boolean purgue() {
		if(this.updates > 1){
			...
			this.updatesHistory = new Stack<History>();
